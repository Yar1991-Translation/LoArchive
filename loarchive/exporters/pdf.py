"""PDF 生成（xhtml2pdf + reportlab 中文字体）与共用的书籍风 HTML 模板。"""

import datetime
import logging

logger = logging.getLogger("loarchive.exporter.pdf")

# Lofter 与 AO3 模板共用的基础样式：页面、正文、封面
_BASE_CSS = """
@page {
    size: A4;
    margin: 2.5cm 2cm;
...
def render_book_html(
    ...
    """渲染共用的书籍风 HTML：封面 + 正文（AO3 模板附加元数据页样式）。"""
...
def _register_chinese_font() -> None:
    try:
        ...
        logger.warning("注册中文字体失败，PDF 可能无法显示中文: %s", e)
...
def generate_lofter_pdf(title, author, author_ip, public_time, url, content, pdf_path) -> bool:
    ...
    <div class="content">
        ...
        html_content = render_book_html(
            title=title or "无标题",
            author=author,
            cover_meta_html=cover_meta_html,
            main_html=main_html,
        )
        ...
        logger.warning("生成 Lofter PDF 失败 %s: %s", pdf_path, e)
        return False
